...
const COMPRESSED_STATE_PATH: &str =
    "data/snga_fractal_gemma_spanish_curriculum_causal_compressed.snga";
const AGENT_COUNT: usize = 5_760;
const PATTERN_SIZE: usize = 12;
const LETTER_PATTERN_SIZE: usize = 7;
const TOP_K: usize = 32;
const TARGET_CAUSAL_EDGES: usize = 500_000;
...
fn main() {
    println!("SNGA fractal curriculum causal compression");
    let mut network = SimplicialNetwork::fractal_3d(config(), fractal_mesh_config());
    match network.load_persistent_state(TRAINED_STATE_PATH) {
        Ok(report) => println!(
    ...
    let start_stats = network.plasticity_stats();
    println!(
        "start: edges={} associative={} causal={} energy={:.1}",
        start_stats.active_edges,
        start_stats.associative_edges,
        start_stats.causal_edges,
        network.total_free_energy()
    );

    let mut chunk = 160_000;
    let mut removed_total = 0;
    while network.plasticity_stats().causal_edges > TARGET_CAUSAL_EDGES && chunk > 0 {
        ...
        if validation_signature(&network) == reference {
            ...
            println!(
                "accepted: removed={} total_removed={} causal={} energy={:.1}",
                removed,
                removed_total,
    ...
    let knowledge_exact = validation_signature(&network) == reference;
    if !knowledge_exact {
        println!("saved=false error=validation signature changed");
        return;
    }

    match network.save_persistent_state(COMPRESSED_STATE_PATH) {
        Ok(report) => println!(
    ...
    println!(
        "final: knowledge_exact={} removed_causal={} edges={} associative={} causal={} energy={:.1}",
        knowledge_exact,
        removed_total,
    ...
    println!(
        ...
        file_size(TRAINED_STATE_PATH),
        file_size(COMPRESSED_STATE_PATH),
        file_size(COMPRESSED_STATE_PATH) as f64 / file_size(TRAINED_STATE_PATH).max(1) as f64
    );
}
