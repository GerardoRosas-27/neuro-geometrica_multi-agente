...
const DEFAULT_INPUT_STATE: &str = "data/snga_fractal_multiregion_substrate.snga";
...
const PATTERN_NODES: usize = 5_760;
const PATTERN_SIZE: usize = 12;
const LETTER_PATTERN_SIZE: usize = 7;
const TOP_K: usize = 32;
const TARGET_LINGUISTIC_ASSOCIATIVE: usize = 500_000;
...
fn main() {
    println!("SNGA multiregion linguistic-only compression");
    let input = env::var("SNGA_COMPRESS_INPUT").unwrap_or_else(|_| DEFAULT_INPUT_STATE.to_string());
    let output =
        env::var("SNGA_COMPRESS_OUTPUT").unwrap_or_else(|_| DEFAULT_OUTPUT_STATE.to_string());
    let linguistic_start = 0;
    let linguistic_end = REGION_SIZE;
    ...
    let removed_associative =
        prune_linguistic_associative(&mut network, &reference, linguistic_start, linguistic_end);
    let removed_causal =
        prune_linguistic_causal(&mut network, &reference, linguistic_start, linguistic_end);
    network.anneal_active_edge_rest_lengths(1.0, 0.0);

    if validation_signature(&network) != reference {
        println!("saved=false error=validation signature changed");
        return;
    }
    ...
    match network.save_persistent_state(&output) {
        Ok(report) => println!(
            ...
            output, report.agents, report.edges, report.causal_edges
        ),
        Err(err) => {
...
fn prune_linguistic_associative(
    ...
    while network.plasticity_stats().associative_edges > TARGET_LINGUISTIC_ASSOCIATIVE && chunk > 0
    {
        let before = network.clone();
        let removed = network.prune_low_value_associative_edges_in_range(chunk, start, end);
        if removed == 0 {
            break;
        }
        if validation_signature(network) == reference {
...
fn prune_linguistic_causal(
    ...
    let mut attempts = 0;
    while chunk > 0 && attempts < 8 {
        attempts += 1;
        let before = network.clone();
        let removed = network.prune_low_value_causal_edges_in_range(chunk, start, end);
        if removed == 0 {
            break;
        }
        if validation_signature(network) == reference {
...
fn legacy_hierarchical_text_pattern(prefix: &str, text: &str, nodes: usize) -> Vec<usize> {
    ...
    let normalized = normalize_text(text);
    for (pos, ch) in normalized.chars().enumerate().take(24) {
        out.extend(legacy_letter_pattern(ch, pos));
    }
...
fn legacy_letter_pattern(ch: char, pos: usize) -> Vec<usize> {
    (0..LETTER_PATTERN_SIZE)
        .map(|offset| {
            ...
            hasher.finish() as usize % PATTERN_NODES
        })
        .collect()
}

fn pattern(prefix: &str, value: &str, _nodes: usize) -> Vec<usize> {
    (0..PATTERN_SIZE)
        .map(|offset| {
            let mut hasher = std::collections::hash_map::DefaultHasher::new();
...
fn regional_pattern(
    ...
    let coding_nodes = REGION_SIZE.min(nodes);
